...
$base_url_for_controller = '/creditor-phys/';
//echo "<pre>";
...
<table class="table my-4">
    ...
    <?php if($rows == $pg){
     ...
     <tr class="text-center">
        ...
        <td class="pt-0"><?= $form->field($each_creditor_phys,'lname')->label('') ?></td>
        <td class="pt-0"><?= $form->field($each_creditor_phys,'fname')->label('') ?></td>
        <td class="pt-0"><?= $form->field($each_creditor_phys,'mname')->label('') ?></td>
        <td class="pt-0"><?= $form->field($each_creditor_phys,'debt_amount')->label('') ?></td>
        <td class="pt-0"><?= $form->field($each_creditor_phys,'inn')->label('') ?></td>
        <td class="border-right pt-0"><?= $form->field($each_creditor_phys,'region')->label('') ?></td>
        ...
        <td class="p-0"><?= Html::submitButton('Save', [
            ...
            'value' => 'save']) ?></td>
        <td class="btn btn-danger p-2 mt-3 ml-1 "><a id='delete_manager' href="<?= $base_url_for_controller . 'delete-creditor-phys/' . '?pg='. $pg . '&page='. $curpage .'&id=' . $each_creditor_phys->id?>" onclick="delete_manager()">delete</a></td>
